...
import cv2
import numpy as np
from model_utils import age_gender_model
...
def predict(img_path):
    ...
    age_gender_models.load_weights('weights/ep10age_gender_299x299.hdf5')
    ...
    label = "{0}  {1}".format(int(pred_identity), "Male" if np.argmax(gender) < 0.5 else "Female")
